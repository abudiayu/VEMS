...
export default function BirthForm({ record, onClose, onSuccess }) {
  ...
  const validate = () => {
    ...
    if (!form.father_name.trim()) {
      errs.father_name = 'Required';
    }

    if (!form.mother_name.trim()) {
      errs.mother_name = 'Required';
    }

    if (!form.kebele.trim()) {
      errs.kebele = 'Required';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        savedRecord = await birthService.update(record.id, form);
      } else {
        savedRecord = await birthService.create(form);
      }

      onSuccess(savedRecord.data);
